...
step() { echo "[$1] $2"; }   # e.g. step "1/6" "checking root..."
...
step "1/6" "repo config: tdnf.conf gpgcheck=${GPGCHECK_GLOBAL:-unset} (full config -> $LOG)"
...
step "2/6" "downloaded $SAMPLE_PKG ok"

GOOD_OUT=$(tdnf install -y --releasever="$RELEASEVER" --installroot="$GOOD_ROOT" "$RPM_FILE" 2>&1)
GOOD_RC=$?
echo "$GOOD_OUT" >> "$LOG"
if [ "$GOOD_RC" -eq 0 ]; then
    step "3/6" "sanity install of UNTOUCHED package: SUCCESS"
    SANITY_OK=1
else
    step "3/6" "sanity install of UNTOUCHED package: FAILED (rc=$GOOD_RC, see $LOG)"
    SANITY_OK=0
fi
...
step "4/6" "tampered copy created (4 bytes flipped @ offset $OFFSET)"
...
step "5/6" "local test repo built (mode=$REPO_MODE)"
...
step "6/6" "install TAMPERED package: rc=$BAD_RC, landed_in_db=$INSTALLED_BAD"

echo "-------------------------------------------"
if [ "$INSTALLED_BAD" -eq 1 ]; then
    echo "VERDICT: VULNERABLE — tampered package was installed!"
elif [ "$SANITY_OK" -ne 1 ] && \
...
elif [ "$SANITY_OK" -eq 1 ] && [ "$BAD_RC" -ne 0 ]; then
    echo "VERDICT: NOT VULNERABLE — good pkg installed, tampered pkg rejected."
else
